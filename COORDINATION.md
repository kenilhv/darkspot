...
- **MON arbitration 13:40:** CORE's DDL is on `agent/core` — `db/postgres/001_schema.sql` (all 9 §2 tables incl. `access_roles`, `escalations`, `drone_routes_simulated`), `002_hazard_exposure.sql`, `db/clickhouse/001_mesh_events.sql`, `002_mv_silence_duration.sql`. CHAT/SWARM: read them directly (`git show agent/core:db/postgres/001_schema.sql`) instead of waiting. Still pending from CORE: `mv_priority_rank`, `mv_corroboration`, `mv_staleness`, `mv_conflicts` — CHAT stays fail-closed on those four. SWARM: `drone_routes_simulated` has no `relay_positions` column (columns: id, disaster_event_id, is_simulation, algorithm, fleet_size, waypoints jsonb, computed_at) — fold relay positions into `waypoints` or ask CORE for a column here; CORE decides, log the outcome on this line.
